...
| **테스트 T** | `Tt` `Tf` `Ta` `Td` `Ts` `To` | 근처 / 파일 / 전체 / 디버그 / 요약 / 도구 창 |
| | `TD` `Tx` | 현재 파일 테스트 디버그 / 테스트 또는 디버그 중지 |
| | `Ty` | 테스트 결과 본문을 시스템 클립보드에 복사 |
| **숫자 별칭** | `1` `2` `3` `4` `5` | `Tx` / `Tf` / `TD` / `Tt` / `Td`와 동일 |
...
Java 테스트를 처음 실행하기 전에는 `:NeotestJava setup`으로 JUnit 실행 파일을 설치한다.
다운로드 확인에서 `Yes, download`를 선택하면 된다. `:Lazy`나 `:Mason` 설치와는 별도다.
`<leader>Tf`는 현재 파일, `<leader>Tt`는 커서 근처 테스트를 실행한다.
기존 숫자 단축키 `<leader>1`~`<leader>5`도 같은 기능의 별칭으로 유지한다.

오른쪽 알림이 사라졌다면 `<leader>nh`로 현재 세션의 알림 기록을 연다.
기록 창에 커서가 들어오므로 `ggVG"+y`로 전체 내용을 시스템 클립보드에 복사할 수 있다.
`q`로 닫는다. 설정을 다시 불러오기 전에는 `:lua Snacks.notifier.show_history()`로도 열 수 있다.
일반 테스트를 실행하면 결과를 담는 대형 테스트 도구 창이 열리고 포커스가 이동한다.
`<leader>To`는 창이 숨겨졌으면 마지막 결과를 열고, 열려 있으면 숨긴다.
결과 창의 `q`나 `<Esc>`도 창만 숨기고 결과는 보존한다.
다시 실행하면 같은 창을 재사용하고 이전 결과 대신 새 결과를 표시한다. 실제 셸 터미널은
기존처럼 `<leader>tt`로 따로 연다.
`<leader>Ty`는 테두리, 제목, 안내 문구를 제외한 테스트 결과 본문만 시스템 클립보드에 복사한다.
