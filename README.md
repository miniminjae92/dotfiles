...
* **Codex**: Global instructions, lifecycle hooks, and custom skills are managed through symlinks under `~/.codex/`. Custom agent TOML files are copied because the Codex loader does not discover them through these symlinks; edit the dotfiles source and run `./install.sh --replace` to update an installed copy. `~/.codex/config.toml` stays local because it contains machine-specific project trust state. Trusted personal work defaults to `danger-full-access` with `on-request` approval; switch to `workspace-write` or `read-only` for untrusted repositories, credentials, production environments, and review-only work. `dotfiles-doctor` verifies local policy, role bindings, and installed agent copies across the default, Google, and Naver Codex homes. A successful doctor run does not replace a fresh-process spawn test. The global Codex hook source lives at `agents/codex/hooks.json`.
...
4.  **Install managed files**
    ...
    The install script links stable Codex instructions, hooks, and custom skills. It copies custom agent TOML files so Codex can discover them. It does not manage Codex config, auth, logs, sessions, caches, system skills, or local state.
...
5.  **Verify the installation**
    ...
    ```
    The doctor also checks the vault snapshot's local tracking alignment. Its
    result is intentionally offline: remote freshness is unverified until a
    snapshot run contacts the remote. Recover pending local tracking work with
    `agent-os-vault-snapshot`, then run `agent-os-vault-snapshot --check` again.
    A manual-check rebase failure exits nonzero so launchd records incomplete
    synchronization. Offline or authentication failures remain best-effort and
    exit successfully after preserving any local commit.
...
6.  **Per-machine setup that remains local**
    ...
    * **A different macOS username breaks the LaunchAgents.** Every plist in
    ...
    the requested command or tool input. The matching `PostToolUse` hook clears
    the oldest resolved request for the same session, turn, and tool without
    storing the tool result. After changing the hook file, open
    ...
    ```
    Normal and watch modes use local notifications only. Disable external delivery without deleting the Keychain item with `agent-notify slack disable`.
