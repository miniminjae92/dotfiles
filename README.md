...
* **Codex**: Global Codex instructions, lifecycle hooks, custom agents, and custom skills are managed through symlinks under `~/.codex/`. `~/.codex/config.toml` stays local because it contains machine-specific project trust state. Trusted personal work defaults to `danger-full-access` with `on-request` approval; switch to `workspace-write` or `read-only` for untrusted repositories, credentials, production environments, and review-only work. `dotfiles-doctor` verifies this local policy across the default, Google, and Naver Codex homes. The global Codex hook source lives at `agents/codex/hooks.json`.
