...
Codex 계약 근거: [공식 Hooks 문서](https://learn.chatgpt.com/docs/hooks).
...
`✓` 검증됨, `?` 미검증(문서/실측 확인 필요), `N/A` 해당 없음/미사용
...
| 최상위 스키마 | `hooks.{Event}[].hooks[]` (중첩) `✓` | `hooks.{Event}[].hooks[]` (중첩, Claude와 동일) `✓` | `{그룹명}.{Event}[]` (**평면형, `hooks` 래퍼 없음**, 임의 그룹명 예: `macos-notification`) `✓` |
...
| 이벤트 집합 | SessionStart, Stop, SubagentStop, UserPromptSubmit, Notification, PreCompact, PreToolUse, PostToolUse, SessionEnd `✓` | SessionStart, SubagentStart, PreToolUse, PermissionRequest, PostToolUse, PreCompact, PostCompact, UserPromptSubmit, SubagentStop, Stop, SessionEnd `✓` | Stop만 사용 확인 `✓` / SessionStart 지원 여부 `?` |
...
| 차단(exit 2) | 지원 (PreToolUse deny 등) `✓` | PreToolUse, PostToolUse, UserPromptSubmit, SubagentStop, Stop에서 지원 `✓`; PermissionRequest는 JSON으로 allow, deny, 미결정 | `?` |
...
이 두 항목이 닫히면 "clerk도 core 인지 대상에 넣을지"를 실측 근거로 재검토할 수 있다(D-004/D-009).
