# CLAUDE.md

@~/.dotfiles/agents/AGENTS.md

## Claude 전용

- 모델 라우팅의 단일 진실 공급원은 `~/.dotfiles/agents/routing.json`이다.
- 윈도우, 주간 캡이 소진되면 작업을 중단하지 말고 Codex(gcodex/ncodex)로 넘기라고 안내한다.
- 커밋 메시지, 분류, 요약 같은 정형 작업(clerk 역할)은 기존 스크립트(`git ai-commit`, agy 경유)를 그대로 사용한다.
- 강결합 편집과 연속 설계 판단은 한 세션에서 유지하고, 독립적, 경계가 명확한 작업만 서브에이전트에 위임한다.
- 메모리는 정본이 아니라 캐시다(2026-08-13 전수 검수). 담는 것은 검증된 함정, 정본 포인터, 전역 행동 피드백 셋뿐이다. 진행 상태("미착수", "대기", "이번 주")와 결정 본문은 담지 않는다. 상태를 적어야 하면 확인 날짜와 "무엇이 바뀌면 이 문장이 거짓이 되는가"를 같은 자리에 적는다. 새 메모리는 반드시 같은 폴더 `MEMORY.md`에 등재한다. 등재되지 않은 파일은 로드되지 않는 죽은 메모리다.
- 메모리는 cwd별로 분리 저장되므로 리포를 옮기거나 이름을 바꾸면 그 사일로가 고아가 된다. 리포 경로가 바뀌면 `~/.claude/projects/<옛 인코딩>/memory/`를 새 사일로로 옮긴다. 격리 보관함은 `~/.claude/memory-archive/`다.

## 매트 포컷 스킬 시험 기간 (2026-07-24 ~ 재검토 전까지)

매트 포컷 스킬은 `~/.dotfiles/agents/skills/`에 **벤더링**해 주력으로 쓴다(active 전량, 스킬별 VENDOR.md, 플러그인 구독은 2026-07-25 해지, D-014). 갱신은 upstream diff를 보고 수동 반영한다. 기능이 겹치면 매트 스킬을 우선한다:

- 세션 인계 → `/handoff` (`handoff-session` 대신. 단, Codex 등 타 CLI로 넘길 땐 기존 `handoff-session` 유지)
- 구현 워크플로 → `/grill-with-docs` → `/to-spec` → `/to-tickets` → `/implement` (단일 세션 규모면 grill 후 바로 `/implement`)
- 한 세션에 안 담기는 안개 낀 대작업 → `wayfinder`(결정 티켓 지도). 출구는 `/to-spec` 합류(`/implement` 직행 금지, upstream 가이드)
- 제품 작업의 전체 틀 → `product-loop`(Claude/Codex 병행 구현, 승격 전). 재개는 자동, 새 사이클 생성은 제안 후 사용자 승인, 애매하면 측정 테스트(D-026). 매트 체인은 통짜가 아니라 분절 삽입: grill+spec→4~5단계, 루프 자체 6~9, tickets+implement→10~11 (D-018)
- 비제품 T1/T2의 기본 틀 → `developer-agent-os`. 표면 없는 인프라/배관은 루프 금지, 마찰기록→D-번호 루프 + 체인 단독 (D-018, D-026)
- superpowers는 전역 비설치. 협업 지정 repo에 project 스코프로만 설치(해당 repo CLAUDE.md에 명시)하고, 그 밖에선 매트 체인 우선 (D-026)
- 언어 공통화(UL 형성, 합의, 정착) → `domain-modeling`/`grill-with-docs`가 repo `CONTEXT.md` + `docs/adr/`에 기록. `term`은 즉석 의미 해명 전용으로 축소(2026-07-24 결정), 파일을 쓰지 않는다
- 새 repo에서 엔지니어링 플로우 첫 사용 전 `/setup-matt-pocock-skills` 1회 실행을 권한다.

기존 스킬 중 `handoff-session`(영속 인계: 산출물 수명이 `/handoff`와 다름)과 `term`은 보존한다. `work`와 세션 아카이브 파이프라인은 실사용 부재로 삭제했다(D-018). 시험 종료 시 이 절을 재검토해 통합 또는 원복한다.
