...
- **저장 후**: 반드시 기록한다. 이걸 빼먹으면 게이트가 같은 커밋을 다시 막는다.
  ...
  생성 시작 시, 대상 diff 를 묶는 아래 메타데이터를 실행해 나온 세 줄을 **변경 없이**
  HTML `<head>` 안에 넣는다. 이 명령 뒤 diff 가 바뀌면 설명서를 다시 만들어야 한다.
  `ack` 는 문서를 고치지 않고 이 값만 검증하므로, 이전 설명서를 현재 커밋에 재사용할 수 없다.

  ```sh
  explain-diff-gate metadata
  ```
